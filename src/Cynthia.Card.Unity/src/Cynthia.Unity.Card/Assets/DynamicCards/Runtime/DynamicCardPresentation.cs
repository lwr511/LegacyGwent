...
namespace Assets.Script.DynamicCards
{
    ...
    public sealed class DynamicCardPresentation : MonoBehaviour
    {
        ...
        public void Tick()
        {
            ...
            float idle = Mathf.SmoothStep(0, 1, Mathf.Clamp01(t - 1));
            float pitch = Mathf.Sin(t * .73f) * 1.5f * idle;
            float yaw = Mathf.Sin(t * .47f) * 7f * idle;
            rect.localRotation = rotation * Quaternion.Euler(pitch, 55 * remaining + yaw, 0);
