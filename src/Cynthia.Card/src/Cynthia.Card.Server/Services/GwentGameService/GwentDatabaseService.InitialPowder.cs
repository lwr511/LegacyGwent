...
namespace Cynthia.Card.Server
{
    public partial class GwentDatabaseService
    {
        ...
        {
            var amount = _initialPowder.Value.Amount; // Validate before creating or mutating a wallet.
            ...
            if (amount > 0 && await accounts.Find(identity & f.Eq("Rewards.RewardId", InitialPowderRewardId)).AnyAsync(cancellationToken))
                return false;
            ...
            if (amount == 0) return false; // Pause without consuming eligibility.
            ...
            var eligible = identity & f.Ne("Rewards.RewardId", InitialPowderRewardId);
        ...
        {
            if (_initialPowder.Value.Amount == 0) return 0;
