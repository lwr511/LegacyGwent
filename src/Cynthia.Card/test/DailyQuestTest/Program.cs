...
class Program
{
    ...
    {
        // Deterministic fixture: rule tests pin the approved daily contract independently of the
        // production DailyQuests.json copied into the build output. GG extends the cap 70 -> 100.
        File.WriteAllText(Path.Combine(AppContext.BaseDirectory,"DailyQuests.json"),
            "{\"LoginPowder\":10,\"GGPowder\":5,\"GGDailyCap\":30,\"Tiers\":["+
            "{\"Crowns\":2,\"Powder\":10},{\"Crowns\":4,\"Powder\":20},{\"Crowns\":6,\"Powder\":30}]}");
        ...
        Check(state.Wallet.Collection.MeteoritePowder==10 && state.Wallet.Collection.DailyQuests.LoginGranted,"24 concurrent login requests grant once");
        ...
        Check(state.DailyCap==100 && state.GGPowder==5 && state.GGDailyCap==30 && state.Tiers.Select(x=>x.Powder).SequenceEqual(new[]{10,20,30}),"approved incremental rewards and GG-inclusive daily cap");
        ...
        Check(state.Wallet.Collection.DailyQuests.Crowns==1 && state.Wallet.Collection.MeteoritePowder==10,"duplicate round callbacks count once");
        state=await db.AwardDailyCrown(user.UserName,"match-a:1",now);
        Check(state.Wallet.Collection.MeteoritePowder==20,"two crowns grant extra ten");
        await db.AwardDailyCrown(user.UserName,"match-b:0",now);
        state=await db.AwardDailyCrown(user.UserName,"match-b:1",now);
        Check(state.Wallet.Collection.MeteoritePowder==40,"four crowns grant extra twenty");
        await db.AwardDailyCrown(user.UserName,"match-c:0",now);
        state=await db.AwardDailyCrown(user.UserName,"match-c:1",now);
        Check(state.Wallet.Collection.MeteoritePowder==70,"six crowns grant extra thirty");
        await Task.WhenAll(Enumerable.Range(0,30).Select(i=>db.AwardDailyCrown(user.UserName,"excess:"+i,now)));
        state=await db.GetDailyQuests(user.UserName);
        Check(state.Wallet.Collection.MeteoritePowder==70 && state.Wallet.Collection.DailyQuests.RoundIds.Count==6,"excess games stop rewards and keep bounded dedup storage");
        now=now.AddMinutes(1);
        await Task.WhenAll(Enumerable.Range(0,24).Select(_=>db.GetDailyQuests(user.UserName)));
        state=await db.GetDailyQuests(user.UserName);
        Check(state.Wallet.Collection.MeteoritePowder==80 && state.Wallet.Collection.DailyQuests.Crowns==0,"midnight resets progress and pays next login once");
        ...
        Check((await restarted.GetDailyQuests(user.UserName)).Wallet.Collection.MeteoritePowder==80,"server restart retains login dedup and wallet");
        ...
        state=await db.GetDailyQuests(user.UserName);
        Check(state.Wallet.Collection.MeteoritePowder==90 && state.Wallet.Collection.OwnedCards.Contains(card),"craft and crown rewards commit without lost wallet updates");
        await users.ReplaceOneAsync(x=>x.Id==user.Id,user);
        Check((await db.GetDailyQuests(user.UserName)).Wallet.Collection.MeteoritePowder==90,"legacy user replacement preserves daily progress");
        now=now.AddDays(-1);
        ...
        Check((await db.GetDailyQuests(other.UserName)).Wallet.Collection.MeteoritePowder==10,"account rewards are isolated");
        ...
        for(int scenario=0;scenario<3;scenario++)
        {
            ...
            game.RoundWon=(w,k,t)=>{winner=w;key=k;return Task.CompletedTask;};
        ...
        Check(ggState.Wallet.Collection.MeteoritePowder==10 && ggState.Wallet.Collection.DailyQuests.GGReceived==0,"GG account starts with login only");
        var ggFirst=await db.AwardDailyGG(ggUser.UserName,"gg-match-1",now);
        Check(ggFirst.Success && ggFirst.Processed && ggFirst.Result.Wallet.Collection.MeteoritePowder==15 &&
            ggFirst.Result.Wallet.Collection.DailyQuests.GGReceived==1 && ggFirst.Result.Wallet.Collection.DailyQuests.GGPowderGranted==5 &&
            ggFirst.Result.Wallet.Collection.DailyQuests.PowderGranted==15,"first GG credits five to the recipient");
        var ggDup=await db.AwardDailyGG(ggUser.UserName,"gg-match-1",now);
        Check(ggDup.Success && !ggDup.Processed && ggDup.Result.Wallet.Collection.MeteoritePowder==15 &&
        ...
        Check(ggState.Wallet.Collection.MeteoritePowder==40 && ggState.Wallet.Collection.DailyQuests.GGReceived==6 &&
        ...
        Check(ggNext.Processed && ggNext.Result.Wallet.Collection.DailyQuests.GGReceived==1 &&
            ggNext.Result.Wallet.Collection.MeteoritePowder==55,"a new day allows a fresh GG");
        now=now.AddDays(-1);
