...
partial class Program
{
    ...
    static async Task FaultBoundaries()
    {
        ...
        Check(error!=null && (await Account(u)).DailyQuests.Crowns==0 && (await Account(u)).MeteoritePowder==10,
        ...
        var hubField=typeof(GwentServerService).GetField("_hub",System.Reflection.BindingFlags.Instance|System.Reflection.BindingFlags.NonPublic);
        var oldHub=hubField.GetValue(server);var notify=await NewUser("notification-failure");error=null;
        hubField.SetValue(server,new FailingHub());
        try{await server.AwardDailyCrown(new User(notify.UserName,"test-connection"),"notify-once",Now);}catch(Exception e){error=e;}
        finally{hubField.SetValue(server,oldHub);}
        Check((await Account(notify)).DailyQuests.Crowns==1,"notification failure happens after reward was persisted");
        Check(error==null,"notification failure must not propagate into the game settlement",error?.Message);
        ...
        using(var c=await WireClient.Connect(url))
        {
            ...
            try{await c.Invoke<UserInfo>("Login",name,"RewardTest2026");}catch(Exception e){error=e;}
            finally{await FailNextWalletWrite(false);}
            var access=await c.Invoke<PremiumCollectionResult>("GetPremiumCollection");
            Check(error==null || access.Status=="unauthenticated","failed login reward must not leave an authenticated half-login",new{loginFailed=error!=null,status=access.Status});
            Check(error!=null,"login fault fixture actually rejects the failed reward write");
            var recovered=await c.Invoke<UserInfo>("Login",name,"RewardTest2026");
            access=await c.Invoke<PremiumCollectionResult>("GetPremiumCollection");
            Check(recovered!=null && access.Success && access.Collection.MeteoritePowder==10,
                "same connection can retry failed login and receives exactly one login reward");
        }
    }
    ...
    static async Task RoundLifecycle()
    {
        ...
        int attempts=0;
        game.RoundWon=(w,key,time)=>{attempts++;return Task.FromException(new Exception("reward storage unavailable"));};
        ...
        Check(attempts==3 && game.PlayersWinCount[0]==2,"permanent reward failure has bounded retries and preserves the winner");

        foreach(bool committed in new[]{false,true})
        {
            var user=await NewUser("round-retry-"+committed);await db.GetDailyQuests(user.UserName);
            game=SimpleGame();game.PlayersWinCount[0]=1;
            winning=game.PlayersDeck[0][0];winning.Status.Conceal=false;winning.Status.Strength=10;game.PlayersPlace[0][0].Add(winning);
            attempts=0;string firstKey=null;DateTimeOffset firstTime=default;bool stable=true;
            game.RoundWon=async(w,key,time)=>
            {
                attempts++;
                if(attempts==1){firstKey=key;firstTime=time;}
                else stable &= key==firstKey && time==firstTime;
                if(committed || attempts>1)await db.AwardDailyCrown(user.UserName,key,Now);
                if(attempts==1)throw new Exception(committed?"response lost after commit":"transient write outage");
            };
            await game.BigRoundEnd();
            Check(attempts==2 && stable && game.CurrentRoundCount==1 && (await Account(user)).DailyQuests.Crowns==1,
                "round retry preserves key/time and credits once after "+(committed?"ambiguous committed write":"transient write failure"));
        }
    }
