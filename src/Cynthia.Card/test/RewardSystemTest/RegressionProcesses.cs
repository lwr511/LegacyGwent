...
partial class Program
{
    ...
    {
        var dotnet=Path.Combine(root,"work/LocalServer/dotnet/dotnet.exe");
        var outcomes=new List<object>();
        foreach(var test in new[]{"DailyQuestTest","PremiumCraftingTest","RewardClientTest"})
        {
            var output=Path.Combine(work,test+".log");
            var args=new List<string>{Path.Combine(root,"src/Cynthia.Card/test",test,"bin/Debug/netcoreapp3.0",test+".dll")};
            if(test=="RewardClientTest")args.Add(Path.Combine(work,"client-results.json"));
            var code=await RunProgram(dotnet,args,output,new Dictionary<string,string>{{"REWARD_TEST_MONGO_URI","mongodb://127.0.0.1:28021"},{"DOTNET_MULTILEVEL_LOOKUP","0"}});
        ...
        try
        {
            foreach(var test in new[]{"Test-DailyQuestsHub","Test-PremiumCraftingHub"})
            {
                ...
                if(test=="Test-PremiumCraftingHub")args.AddRange(new[]{"-MongoUri","mongodb://127.0.0.1:28021/gwentdiy"});
    ...
    {
        var provider=new ServiceCollection().AddSingleton<IMongoClient>(new MongoClient("mongodb://127.0.0.1:28021")).AddSingleton(new InitialPowderOptions(0)).BuildServiceProvider();
    ...
    static async Task ConfigurationCases()
    {
        ...
        foreach(var item in cases)
        {
            ...
            int code=await RunProgram(Path.Combine(root,"work/LocalServer/dotnet/dotnet.exe"),
