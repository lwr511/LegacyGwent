...
partial class Program
{
    ...
    static async Task PremiumCopyCases()
    {
        ...
        using (var unauth = await WireClient.Connect(url))
        {
            ...
            Check(stored.PremiumCards[bronze] == 1 && stored.Deck.Count == 3, "per-deck copy selection persists in MongoDB");
            ...
            var player = new SinkPlayer { Deck = stored, PremiumCards = new HashSet<string> { bronze, leader } };
            ...
            using (var reconnect = await WireClient.Connect(url))
            {
                ...
                Check(login.Decks.Single(x => x.Id == deck.Id).PremiumCards.Count == 0 && CardInventory.PremiumCount(reloaded.Collection, bronze) == 3,
