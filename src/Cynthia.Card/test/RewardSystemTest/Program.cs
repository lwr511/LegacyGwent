...
public class SuiteStartup
{
    ...
    public void ConfigureServices(IServiceCollection services)
    {
        ...
        services.AddSingleton<IMongoClient>(new MongoClient("mongodb://127.0.0.1:28021"));
        services.RemoveAll<InitialPowderOptions>();
        services.AddSingleton(new InitialPowderOptions(InitialAmount));
        // Background season/migration jobs are unrelated to rewards and are omitted from this isolated host.
        foreach(var d in services.Where(x=>x.ServiceType==typeof(IHostedService) &&
            (!RunInitialBackfill || x.ImplementationType!=typeof(InitialPowderGrantService))).ToArray())services.Remove(d);
    }
...
partial class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        try
        {
            ...
            await Group("database failures and notification isolation",FaultBoundaries);
    ...
    static async Task NetworkDaily()
    {
        ...
        using(var unauth=await WireClient.Connect(url))
        {
            ...
            Check((await Account(user)).MeteoritePowder==10,"LAN successful login pays before task endpoint is called");
            ...
            Check(refresh.All(x=>x.Wallet.Collection.MeteoritePowder==10),"32 LAN refreshes do not repeat login reward");
            await a.Invoke<UserInfo>("Login",user.UserName,"RewardTest2026");
            Check((await Account(user)).MeteoritePowder==10,"same-connection relogin does not repeat reward");
            Now=Now.AddSeconds(1);
            state=await a.Invoke<DailyQuestResult>("GetDailyQuests");
            Check(state.Wallet.Collection.MeteoritePowder==20 && state.Wallet.Collection.DailyQuests.PowderGranted==10,"online client refresh across China midnight grants exactly next login");
            ...
            long[] expected={20,30,30,50,50,80};
            for(int i=0;i<6;i++)
            {
            ...
            state=await a.Invoke<DailyQuestResult>("GetDailyQuests");
            Check(state.Wallet.Collection.MeteoritePowder==80 && state.Wallet.Collection.DailyQuests.PowderGranted==70 && state.Wallet.Collection.DailyQuests.Crowns==6,"80 excess settlements cannot exceed daily 70 powder / 6 crowns");
            Check((await Account(other)).MeteoritePowder==10,"other connected account unchanged until its own refresh");
            Check((await b.Invoke<DailyQuestResult>("GetDailyQuests")).Wallet.Collection.MeteoritePowder==20,"other online account gets its own next-day login once");
            ...
            Check((await Account(user)).MeteoritePowder==80,"forged reward requests leave wallet unchanged");
        }
    }
    static async Task Calendar()
    {
        foreach(string boundary in new[]{"2026-10-01T00:00:00+08:00","2027-01-01T00:00:00+08:00","2028-02-29T00:00:00+08:00","2028-03-01T00:00:00+08:00"})
        {
            ...
            Check(state.Wallet.Collection.MeteoritePowder==20 && state.Wallet.Collection.DailyQuests.Day==zero.ToString("yyyy-MM-dd"),"calendar transition "+boundary);
            await db.GetDailyQuests(u.UserName);
            Check((await Account(u)).MeteoritePowder==20,"calendar repeated refresh remains once "+boundary);
        }
        ...
        Check(next.Wallet.Collection.MeteoritePowder==20,"ten offline days do not accumulate unclaimed daily bonuses");
        for(int day=0;day<7;day++){Now=Now.AddDays(1);await db.GetDailyQuests(user.UserName);}
        Check((await Account(user)).MeteoritePowder==90,"seven consecutive later login days each pay exactly ten");
        ...
        Now=Now.AddDays(2);
        Check((await db.GetDailyQuests(user.UserName)).Wallet.Collection.MeteoritePowder==90,"clock recovery does not double-pay latest day");
    ...
    static async Task ProcessingDay()
    {
        ...
        Check(state.Wallet.Collection.DailyQuests.Crowns==0 && state.Wallet.Collection.MeteoritePowder==20,
        ...
        Check(account.DailyQuests.Crowns==2 && account.MeteoritePowder==30,
        ...
        Check(state.Wallet.Collection.DailyQuests.Crowns==0 && state.Wallet.Collection.MeteoritePowder==80,
        ...
        Check(samples>=2 && state.Wallet.Collection.DailyQuests.Day=="2026-09-26" && state.Wallet.Collection.DailyQuests.Crowns==1 && state.Wallet.Collection.MeteoritePowder==20,
        ...
        Check(state.Wallet.Collection.DailyQuests.Crowns==1 && state.Wallet.Collection.MeteoritePowder==20,
    ...
    static async Task Concurrent()
    {
        ...
        Check((await Account(u)).MeteoritePowder==10,"128 simultaneous first logins create one wallet and one reward");
        ...
        Check((await Account(u)).DailyQuests.Crowns==1 && (await Account(u)).MeteoritePowder==10,"128 repeated same-round callbacks count one crown");
        await Task.WhenAll(Enumerable.Range(1,128).Select(i=>db.AwardDailyCrown(u.UserName,"unique:"+i,Now)));
        var account=await Account(u);
        Check(account.MeteoritePowder==70 && account.DailyQuests.Crowns==6 && account.DailyQuests.PowderGranted==70,"128 competing unique wins stop exactly at daily cap");
        ...
        Check(account.MeteoritePowder==80 && account.DailyQuests.Crowns==1,"concurrent day reset, login and duplicated crown settle once");
    }
    static async Task<(int code,string text)> Grant(string player,long amount,string reward,string reason="Reward suite")
    {
        ...
        foreach(string a in new[]{"-NoProfile","-NonInteractive","-ExecutionPolicy","Bypass","-File",Path.Combine(root,"scripts/Grant-MeteoritePowder.ps1"),"-PlayerId",player,"-Amount",amount.ToString(),"-RewardId",reward,"-Reason",reason,"-MongoUri","mongodb://127.0.0.1:28021/gwentdiy"})start.ArgumentList.Add(a);
    ...
    static async Task Admin()
    {
        ...
        Check((await db.GetDailyQuests(a.UserName)).Wallet.Collection.MeteoritePowder==260,"first login retains offline grant and adds only ten");
        var duplicate=await Grant(a.Id,250,"offline");
        Check(duplicate.code==0 && (await Account(a)).MeteoritePowder==260 && (await Account(a)).Rewards.Count==1,"retry same reward ID does not duplicate powder or ledger");
        await Grant(a.Id,999,"offline","different payload");
        Check((await Account(a)).MeteoritePowder==260 && (await Account(a)).Rewards.Single().Amount==250,"same reward ID with altered amount cannot regrant");
        ...
        Check(jobs.All(x=>x.code==0) && (await Account(a)).MeteoritePowder==277 && (await Account(a)).Rewards.Count(x=>x.RewardId=="concurrent")==1,"ten real concurrent administrator processes grant one duplicate reward");
        jobs=await Task.WhenAll(Enumerable.Range(0,8).Select(i=>Grant(a.Id,3,"distinct-"+i)));
        Check(jobs.All(x=>x.code==0) && (await Account(a)).MeteoritePowder==301,"eight distinct concurrent grants all accumulate without loss");
        ...
        Check(name.code!=0 && (await Account(a)).MeteoritePowder==302,"username is not confused with player ID");
        foreach(long value in new long[]{0,-1,1000000001})
        {var bad=await Grant(a.Id,value,"bad-"+value);Check(bad.code!=0 && (await Account(a)).MeteoritePowder==302,"invalid administrator amount rejected: "+value);}
        ...
        Check(final.MeteoritePowder==862 && final.DailyQuests.PowderGranted==70,"administrator rewards may exceed daily cap without resetting or consuming daily quota");
        await db.AwardDailyCrown(a.UserName,"post-admin-extra",Now);
        Check((await Account(a)).MeteoritePowder==862,"administrator grant does not reopen exhausted daily rewards");
    ...
    static async Task Composition()
    {
        ...
        Check(account.MeteoritePowder==510-100+60+40,"concurrent craft, daily tiers and eight administrator grants conserve exact balance");
        Check(account.DailyQuests.PowderGranted==70 && account.OwnedCards.Contains(card),"concurrent transactions preserve cap and permanent ownership together");
        await Users.ReplaceOneAsync(x=>x.Id==u.Id,u);
        Check((await Account(u)).MeteoritePowder==510 && (await Account(u)).DailyQuests.Crowns==6,"legacy user replacement cannot overwrite independent wallet");
        await db.CraftPremium(u.UserName,card);await db.AwardDailyCrown(u.UserName,"after-spending",Now);
        Check((await Account(u)).MeteoritePowder==510,"spending and repeated crafting do not reopen capped daily rewards");
        var broke=await NewUser("exact-cost");await db.GetDailyQuests(broke.UserName);
        await Grant(broke.Id,90,"exact");var crafted=await db.CraftPremium(broke.UserName,card);
    ...
    static async Task RoundSettlement()
    {
        ...
        try
        {
            ...
            for(int scenario=0;scenario<3;scenario++)
            {
                ...
                int calls=0;game.RoundWon=async(w,k,t)=>{calls++;await db.AwardDailyCrown(w==0?a.UserName:b.UserName,k,t);};
    ...
    static async Task Restart()
    {
        ...
        using(var c=await WireClient.Connect(url))
        {
            ...
            Check(state.Wallet.Collection.MeteoritePowder==373 && state.Wallet.Collection.DailyQuests.Crowns==4,"restarted ASP.NET host retains exact wallet and crown tiers over LAN");
            await db.AwardDailyCrown(name,"persist:3",Now);await Grant(user.Id,333,"restart-admin");
            Check((await Account(user)).MeteoritePowder==373,"round and administrator retry remain idempotent after host restart");
        }
        File.WriteAllText(Path.Combine(work,"mongo-restart.request"),"restart isolated port 28021");
        ...
        using(var c=await WireClient.Connect(url))
        {
            await c.Invoke<UserInfo>("Login",name,"RewardTest2026");
            var state=await c.Invoke<DailyQuestResult>("GetDailyQuests");
            Check(state.Wallet.Collection.MeteoritePowder==383 && state.Wallet.Collection.DailyQuests.PowderGranted==10 && state.Wallet.Collection.DailyQuests.Crowns==0,"day reset after database restart preserves wallet and pays new login once");
            await db.AwardDailyCrown(name,"persist:3",Now.AddDays(-1));
            state=await c.Invoke<DailyQuestResult>("GetDailyQuests");
            Check(state.Wallet.Collection.MeteoritePowder==383 && state.Wallet.Collection.DailyQuests.Crowns==0,
