...
partial class Program
{
    ...
    static async Task InitialPowderCases()
    {
        ...
        {
            File.WriteAllText(path, "{\"Amount\":" + value + "}");
            Check(InitialPowderOptions.Load(path).Amount == value, "valid configurable amount " + value);
        }
        ...
        Check(wallet.MeteoritePowder == 4910 && wallet.DailyQuests.Crowns == 1 && wallet.OwnedCards.Count == 1 && InitialGrants(wallet) == 1,
        ...
        using (var client = await WireClient.Connect(url))
        {
            ...
            Check(response.Collection.MeteoritePowder == 5010 && InitialGrants(response.Collection) == 1,
                "real login and client wallet response contain 5000 initial plus 10 daily powder");
        }
        await host.StopAsync(); host.Dispose(); host = null;
        await StartHost();
