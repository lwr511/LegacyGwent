...
for kind,bridge,prefix in [('Native','Bridge2022','native'),('Legacy2017','LegacyBridge2019','legacy')]:
    ...
    for ident in selected:
        for item in audio_map.get(ident,{}).get('files',[]):
            ...
            if not wav.exists():
                subprocess.run([r'<USER_HOME>/Documents/Codex/2026-09-05/c-steamlibrary-steamapps-common-thronebreaker-the/work/vgmstream/vgmstream-cli.exe','-o',str(wav),str(wav.with_suffix('.wem'))],check=True,stdout=subprocess.DEVNULL)
