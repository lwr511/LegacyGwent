...
out=root/'LegacyAnimationData';out.mkdir(exist_ok=True)
needed=set((root/'legacy_needed.txt').read_text().split())
...
for o in env.objects:
    ...
    for ref in controller['m_AnimationClips']:
        ...
        st=clip['m_StreamedClip'];de=clip['m_DenseClip'];cn=clip['m_ConstantClip']['data'];stream=st['curveCount'];dense=de['m_CurveCount']
        ...
        for binding in t['m_ClipBindingConstant']['genericBindings']:
            ...
            offset+=dim
        key=match[0]+'_'+str(zlib.crc32(clip_obj.assets_file.name.encode()))+'_'+str(clip_obj.path_id)
