...
import json,re,zlib
w=Path(__file__).resolve().parent;p=Path('C:/UnityProjects/LegacyGwent/src/Cynthia.Card.Unity/src/Cynthia.Unity.Card');ids={x['id'] for x in json.loads((p/'Assets/DynamicCards/Content/catalog.json').read_text())['cards'] if '/Old/Thronebreaker/' in x['prefab']};records=json.loads((w.parent/'NativeAnimationData/animations.json').read_text())['animators'];issues=[];checked=0
for r in {(r['id'],r['path']):r for r in records}.values():
 if r['id'] not in ids or not r['clips']:continue
 prefix='Source_'+format(zlib.crc32(r['path'].encode()),'08x');folder=p/'Assets/DynamicCards/Content/Old/Thronebreaker'/r['id'];f=folder/(prefix+'.controller')
...
(w/'native-controller-audit.json').write_text(json.dumps({'checked':checked,'issues':issues},indent=2));print('controllers',checked,'issues',len(issues));print(issues[:10])
