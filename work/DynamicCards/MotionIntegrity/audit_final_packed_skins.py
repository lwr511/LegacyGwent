...
sys.path.insert(0,'<USER_HOME>/Documents/Codex/2026-09-05/c-steamlibrary-steamapps-common-thronebreaker-the/work/python_deps')
import UnityPy
from UnityPy.helpers.MeshHelper import MeshHandler
w=Path(__file__).resolve().parent;root=w/'FinalHarness/Library/DynamicCardsBundles/StandaloneWindows64';idx=json.loads((root/'cards.index.json').read_text());report=[];checked=0
for part in idx['parts']:
 ...
 (w/'final-packed-skin-audit.json').write_text(json.dumps({'complete':False,'checked':checked,'issues':report},indent=2));print('PACKED_SKIN',part['file'],'checked',checked,'issues',len(report),flush=True);del env;gc.collect()
(w/'final-packed-skin-audit.json').write_text(json.dumps({'complete':True,'checked':checked,'issues':report},indent=2))
