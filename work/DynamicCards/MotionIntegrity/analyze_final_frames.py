...
for f in (w/'FinalFrames').glob('*-large.png'):
...
rows.sort(key=lambda r:r['correlation']);(w/'final-visual-metrics.json').write_text(json.dumps(rows,indent=2));print('Analyzed',len(rows));print('lowest correlations',[(r['art'],round(r['correlation'],3),round(r['brightnessRatio'],2)) for r in rows[:12]])
