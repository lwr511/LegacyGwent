...
files=sorted((w/'FinalFrames').glob('*-large.png'))
for page in range((len(files)+47)//48):
 ...
 for i,f in enumerate(files[page*48:page*48+48]):
  art=f.name[:-10];override=w/'FinalEdgeFrames'/(art+'-large-5.png');f=override if art in ('12210500','203222') and override.exists() else f;m=entries[art].get('topMargin',0);side=math.ceil(m*648/947*.5);im=Image.open(f).convert('RGB');scale=im.width/1024
 ...
 sheet.save(w/('final-cards-%02d.jpg'%page),quality=90)
