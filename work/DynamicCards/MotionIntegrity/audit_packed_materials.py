...
import UnityPy
w=Path(__file__).resolve().parent;root=w/'Harness/Library/DynamicCardsBundles/StandaloneWindows64';idx=json.loads((root/'cards.index.json').read_text());canonical=json.loads((w.parent/'source_null_material_slots.json').read_text())['slots'];allowed={(x['source'],x['type'],'Card/'+x['path'],x['slot']) for x in canonical};issues=[];checked=0
...
for part in idx['parts']:
 ...
 del env;gc.collect()
(w/'packed-material-audit.json').write_text(json.dumps({'checkedRenderers':checked,'issues':issues},indent=2));print('renderers',checked,'issues',len(issues));print(issues[:12])
