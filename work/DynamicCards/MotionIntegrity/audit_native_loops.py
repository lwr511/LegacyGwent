...
w=Path('C:/UnityProjects/LegacyGwent/work/DynamicCards/MotionIntegrity');p=Path('C:/UnityProjects/LegacyGwent/src/Cynthia.Card.Unity/src/Cynthia.Unity.Card');ids={x['id'] for x in json.loads((p/'Assets/DynamicCards/Content/catalog.json').read_text())['cards'] if '/Old/Thronebreaker/' in x['prefab']};records=json.loads((w.parent/'NativeAnimationData/animations.json').read_text())['animators'];rows=[];missing=[];checked=0;seen={}
for r in records:
 if r['id'] not in ids:continue
 prefix='Source_'+format(zlib.crc32(r['path'].encode()),'08x');folder=p/'Assets/DynamicCards/Content/Old/Thronebreaker'/r['id']
...
report={'checked':checked,'changes':rows,'missing':missing};(w/'native-loop-audit.json').write_text(json.dumps(report,indent=2));print('checked',checked,'mismatches',len(rows),'unmatched',len(missing));print(rows[:6]);print(missing[:6])
