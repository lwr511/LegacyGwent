...
for kind,ident,src in [('Legacy','13221601',r'E:\Hbackup\FileRecv\Gwent\Gwent\Gwent_Data\StreamingAssets\AssetBundles\cardassets\scenes')]:
...
(out:=w/'TimingRepair/additional-toad-clips.json').write_text(json.dumps(report,indent=2));print('RESTORED',[(r['path'],[(s['name'],s['clips']) for l in r['layers'] for s in l['states']]) for r in report])
