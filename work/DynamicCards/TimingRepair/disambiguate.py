...
sys.path.insert(0,r'<USER_HOME>/Documents/Codex/2026-09-05/c-steamlibrary-steamapps-common-thronebreaker-the/work/python_deps')
import UnityPy
...
for kind,ident,src in [('Native','11590100',r'C:\SteamLibrary\steamapps\common\Thronebreaker The Witcher Tales\Thronebreaker_Data\StreamingAssets\bundledassets\cardassets\scenes'),('Legacy','13230201',r'E:\Hbackup\FileRecv\Gwent\Gwent\Gwent_Data\StreamingAssets\AssetBundles\cardassets\scenes')]:
 original=w/(kind+'AnimationData');records=[r for r in json.loads((original/'animations.json').read_text())['animators'] if r['id']==ident];by_path={r['path']:r for r in records};dest=w/'TimingRepair'/kind;dest.mkdir(exist_ok=True)
 for r in records:
  for c in r['clips']:
...
(out:=w/'TimingRepair/duplicate-clips.json').write_text(json.dumps(report,indent=2));print('RESTORED',[(r['path'],[(s['name'],s['clips']) for l in r['layers'] for s in l['states']]) for r in report])
