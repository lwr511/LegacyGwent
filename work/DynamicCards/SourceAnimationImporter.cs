        ...
        foreach(var group in doc.animators.GroupBy(r=>new { r.id, r.path }).Select(records=>records.Last()).GroupBy(r=>r.id))
        {
            ...
            try
            {
                ...
                foreach(var record in group)
                {
                    ...
                    record.clips=record.clips.GroupBy(c=>c.file).Select(g=>g.First()).ToArray();
                    ...
                    if(record.layers==null || record.layers.Length==0)throw new InvalidOperationException(record.id+": source controller contract missing; run build_animation_contracts.py first.");
