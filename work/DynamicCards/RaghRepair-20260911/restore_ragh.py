"""Restore Ragh Nar Roog's animated source; dry-run unless --apply is given."""
...
entry = next(c for c in prior['cards'] if '11310100' in c.get('artIds', []))
assert entry['id'] == '10400101'
entry.update(sourceVersion='Latest', sourceId='10400101')
existing = [c for c in data['cards'] if '11310100' in c.get('artIds', [])]
assert len(existing) == 1 and existing[0]['id'] in ('11310101', '10400101')
queue = [backup / 'Latest/10400101/Card.prefab', backup / 'Latest/Audio/715479366.wav']
queue += [p for p in (backup / 'Latest/10400101').iterdir() if p.is_file() and p.suffix != '.meta']
