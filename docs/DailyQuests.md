...
- 该首场资格同时适用于普通匹配和密码真人房；投降或断线的真人对局也会占用当天与该对手的首场资格。此限制按服务端 `GameResults` 对局记录判断，不另建反刷表。
...
配置位于 `src/Cynthia.Card/src/Cynthia.Card.Server/DailyQuests.json`，服务器启动后缓存配置；改动后需重启服务。当前值为 `LoginPowder=20`、`Tiers=2/4/6 冠 25/35/45`、`GGPowder=5`、`GGDailyCap=30`（合计 155）。旧版缺少 GG 字段的测试配置会按 5／30 默认值加载。GG 单次奖励和上限必须为正数，且上限为单次奖励的整数倍；显式零或负数会被拒绝。当前界面按 6 个小局刻度（3 个完整王冠）、3 档加 GG 设计。

## 服务端与持久化

同对手规则的独立验收见 [SameOpponentRewardsVerification.md](SameOpponentRewardsVerification.md)：复用战绩查询、跨日并发、真实联网 GG 及四语言页面验证。
...
对局结束时 `GwentMatchs` 先向 `GwentServerService.RecordFinishedMatch` 登记服务器场次回执（仅限两个不同账号的真人局），再把结果发回客户端。`GwentHub.SendGG(MyName, EnemyName)` 保持老客户端的两字符串签名，但 `GwentServerService.SendGG` 改为用 `Context.ConnectionId` 解析真实发送者，并只接受“最近一场真实完成对局”的对手：接收者用户名、场次 id 与结算时间全部来自回执，调用方名字只用于比对对手显示名，不参与授权。人机、自我、非参赛者与未完成对局没有回执，因此不会发奖。
...
GG 发奖的持久去重账本与临时对局回执分开：回执保留每个账号最近一场真人对局、有效期 6 小时。重启会清空尚未使用的回执，重启前尚未发送的 GG 不能补领；已发奖励与去重账本仍保存在数据库中。GG 提示和社交计数属于发奖后的通知，失败不会回滚粉尘；当前没有通知／社交计数的自动重试队列。
...
- 任务页底部独立显示首场真人对手规则，持续可见，不会被完成、同步或错误状态覆盖；详细的投降／断线说明保留在本页文档。
...
2026-09-21 最终独立验收：服务器规则 54 项、每日任务 34 项、闪卡合成 21 项、客户端同步 21 项、真实 SignalR 双账号匹配／GG 71 项全部通过；Unity 真实到账界面及 1920×1080、960×540 四语言画面已检查。详见 [GG 奖励验收记录](GGRewardsVerification.md)。后续段落保留早期检查的覆盖范围。
...
2026-09-21 新增对局结束 GG 奖励。`src/Cynthia.Card/test/GGRewardsTest` 的 54 项检查全部通过：接收方每个有效 GG 得 5 粉、6 次封顶 30、重复与 32 路并发同场次只结算一次、24 路并发唯一场次不超上限、跨中国零点重置、封顶场次跨日重放不补发、服务重建后账本仍去重、旧钱包升级保留 GG 进度，以及两个 JSON 路径均隐藏持久化账本；社交 GG 计数只在首次有效 GG 加一（重复不加、离线仍加、封顶后的有效 GG 仍各加一）。同时用真实 `GwentServerService.SendGG` 复验了无认证连接、伪造发送者、非参赛者、任意对手名、自我、无已完成对局、断线接收方与上限封顶。`DailyQuestTest` 已同步为 GG 合计上限 100（34 项，含 GG 用例）。GG 授权用例由内存连接表直接驱动真实服务方法，未经过 SignalR 传输，也不等同于完整双真人联网对局。
...
$env:REWARD_TEST_MONGO_URI='mongodb://127.0.0.1:28020'
./work/LocalServer/dotnet/dotnet.exe ./src/Cynthia.Card/test/GGRewardsTest/bin/Debug/netcoreapp3.0/GGRewardsTest.dll
```
